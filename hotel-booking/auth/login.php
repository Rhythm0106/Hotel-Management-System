...
if(isset($_POST['submit'])){
  if(empty($_POST['email']) OR empty($_POST['password'])) {
    echo "<script>alert('Atleast one input is Empty')</script>";
  } else {
		$email = $_POST['email'];
  ...
  $login ->execute();
   ...
   //GET THE ROW COUNT 
   if($login->rowCount() > 0) { // row count will grab the affected data by the number of affected rows 
      if(password_verify($password, $fetch['mypassword'])){
       // echo "<script>alert('LOGGED IN')</script>";
        $_SESSION['username'] = $fetch['username'];
        $_SESSION['id'] = $fetch['id'];    //Defining session varoables
